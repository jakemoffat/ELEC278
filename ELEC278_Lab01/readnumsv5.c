// readnumsv6.c - Lab 01 - Jake Moffat
// READNUMSV5.C - Lab01 - David Athersych
//
...
int delete_data (int delete) {
//delete an item of data at a given point
    struct node *pn = pnew_node (delete);
    struct node *pdeleted;
    struct node *pmove;
    struct node *pend;
    ...
    while (pmove != NULL) {
        if (pmove->datavalue == delete) {
            if (pmove->pnextnode == NULL) {
                pdeleted = pend;
                pend->pnextnode = NULL;
                break;
            }
                ...
                pdeleted = phead;
                phead = pmove->pnextnode;
                break;
            }
            else {
                pdeleted = pmove;
                pend->pnextnode = pmove->pnextnode;
                break;
            }
        }

        pend = pmove;
        pmove = pmove->pnextnode;
...
void main (void)
{
    ...
    printf ("To delete a number, enter letter D followed by a number already inputted.\n");
